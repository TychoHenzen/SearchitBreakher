...
public class GoxFileFormatTests
{
    ...
    public void LoadingTextAndGoxFiles_ProducesSameVoxelData()
    {
        ...
        Assume.That(File.Exists(textFilePath), Is.True, $"Test text file not found at {textFilePath}");
        Assume.That(File.Exists(goxFilePath), Is.True, $"Test GOX file not found at {goxFilePath}");

        // Act
        var textVoxels = LoadVoxelTextFile(textFilePath);
        var goxChunk = ChunkLoader.LoadGoxFile(goxFilePath, Vector3.Zero);
        Assume.That(goxChunk, Is.Not.Null);
        var goxVoxels = goxChunk.GetVoxelData();

        // Output non-zero voxels from GOX file
        for (int i = 0; i < goxVoxels.Length; i++)
        {
            if (goxVoxels[i] <= 0) continue;

            var pos = TestHelpers.GetPosition(i);
        ...
        // Assert
        var areEqual = textVoxels.SequenceEqual(goxVoxels);
        ...
        if (!areEqual)
        {
            ...
            for (int i = 0; i < minLength; i++)
            {
                ...
                if (differenceCount < 20) continue;
    ...
    private static byte MapColorToVoxelType(int color)
    {
        ...
        color &= 0x00FFFFFF; // Remove any alpha channel
